/**
 * Supabase Edge Function: elevated-check
 *
 * Runs every 5 minutes and sends a push notification to any user whose most
 * recent CGM reading is above their configured elevated-alarm threshold.
 *
 * Flow per user:
  ...
  const res = await fetch(`https://fcm.googleapis.com/v1/projects/${projectId}/messages:send`, {
    ...
    body: JSON.stringify({
      message: {
        ...
        android: { priority: "high", notification: { sound: "default", channel_id: "elevated_alarm" } },
      },
    }),
  });
...
async function sendApnsPush(
  ...
  const payload = JSON.stringify({
    aps: { alert: { title, body }, sound: "glev_elevated.wav", badge: 1, "interruption-level": "time-sensitive", "content-available": 1 },
  });
...
Deno.serve(async (_req: Request) => {
  ...
  if (alarmError) {
    console.error("[elevated-check] failed to fetch alarm settings:", alarmError.message);
  ...
  if (tokenError) {
    console.error("[elevated-check] failed to fetch push tokens:", tokenError.message);
