  ...
  for (const user of users) {
    try {
      ...
      const { data: cgmRows, error: cgmError } = await sb
        ...
        .limit(1);
      if (cgmError) console.error(`[hyper-check] cgm_samples error for ${user.user_id}:`, cgmError.message);
      ...
      const { data: ahRows, error: ahError } = await sb
        ...
        .order("timestamp", { ascending: false })
        .limit(1);
      if (ahError) console.error(`[hyper-check] apple_health_readings error for ${user.user_id}:`, ahError.message);
      ...
      if (candidates.length === 0) { console.log(`[hyper-check] no recent CGM data for ${user.user_id}, skipping`); continue; }
      ...
      sent++;
      console.log(`[hyper-check] sent push to ${user.user_id} (${user.push_platform}), value=${latestValue}, threshold=${threshold}`);
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      errors.push(`${user.user_id}: ${msg}`);
      console.error(`[hyper-check] error for user ${user.user_id}:`, msg);
    }
  }
