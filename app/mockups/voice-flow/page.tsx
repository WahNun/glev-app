...
function AtomMark({ size = 18 }: { size?: number }) {
  ...
  return (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" aria-hidden>
      ...
      <ellipse cx="12" cy="12" rx="9" ry="3.6" stroke={ACCENT} strokeWidth="1.2" />
      <ellipse cx="12" cy="12" rx="9" ry="3.6" stroke={ACCENT} strokeWidth="1.2" transform="rotate(60 12 12)" />
      <ellipse cx="12" cy="12" rx="9" ry="3.6" stroke={ACCENT} strokeWidth="1.2" transform="rotate(120 12 12)" />
    </svg>
  );
}

function NavRow({ icon, label, active = false }: { icon: React.ReactNode; label: string; active?: boolean }) {
  return (
    <div style={{
      ...
      <span style={{ display: "inline-flex", width: 16, height: 16, color: active ? ACCENT : TEXT_FAINT }}>{icon}</span>
      <span>{label}</span>
    </div>
  );
}

const SVG_DASH = (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>);
const SVG_HISTORY = (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 3v6h6"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L3 9"/><path d="M12 7v5l3 2"/></svg>);
...
const SVG_SIGNOUT = (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>);
    ...
    <div style={{
      ...
      <aside style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 10px", marginBottom: 24 }}>
          <AtomMark size={20} />
          <span style={{ fontSize: 18, fontWeight: 800, letterSpacing: "-0.03em" }}>glev<span style={{ color: ACCENT }}>.</span></span>
        </div>
        ...
        <div style={{ marginTop: "auto", padding: "9px 11px", display: "flex", alignItems: "center", gap: 10, color: TEXT_FAINT, fontSize: 12 }}>
          <span style={{ display: "inline-flex", width: 14, height: 14 }}>{SVG_SIGNOUT}</span>
          <span>Sign Out</span>
        </div>
      </aside>
      ...
      <aside style={{
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10 }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: "0.08em", color: "rgba(255,255,255,0.45)" }}>
              AI FOOD PARSER <span style={{ fontWeight: 500, color: ACCENT, letterSpacing: "0.02em" }}>GPT-Begründung</span>
            </div>
          </div>
          <div style={{
            ...
            <span style={{ width: 6, height: 6, borderRadius: 99, background: MINT }} />
            BEREIT
          </div>
        </div>
