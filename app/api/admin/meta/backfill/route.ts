...
function mapLeadFields(lead: any) {
  ...
  const norm = (s: string) =>
    String(s || "").toLowerCase().replace(/[äöüß]/g, (m) => ({ ä: "ae", ö: "oe", ü: "ue", ß: "ss" }[m] || m));
  ...
  const is_test = fd.some((f) =>
    (Array.isArray(f?.values) ? f.values : []).some((v: unknown) => String(v).includes("<test lead:")),
  );
...
export async function POST(req: Request) {
  ...
  if (!PAGE_ACCESS_TOKEN || !PAGE_ID) {
    return Response.json({ error: "META_PAGE_ACCESS_TOKEN or META_PAGE_ID not set" }, { status: 500 });
  }

  const results: object[] = [];

  // 1. Alle Formulare der Seite abrufen
  const formsRes = await fetch(`${GRAPH}/${PAGE_ID}/leadgen_forms?limit=20`, {
    headers: { Authorization: `Bearer ${PAGE_ACCESS_TOKEN}` },
  });
  const formsJson = await formsRes.json();
  if (!formsRes.ok) {
    return Response.json({ error: "leadgen_forms fetch failed", detail: formsJson }, { status: 502 });
  }

  const forms: { id: string; name: string }[] = formsJson?.data ?? [];

  // 2. Für jedes Formular die Leads abrufen
  for (const form of forms) {
    let cursor = `${GRAPH}/${form.id}/leads?fields=${LEAD_FIELDS}&limit=100`;
    while (cursor) {
      ...
      if (!leadsRes.ok) {
        results.push({ form_id: form.id, error: leadsJson });
        break;
      }

      const leads: any[] = leadsJson?.data ?? [];
  ...
  return Response.json({ ok: true, forms_found: forms.length, leads_processed: results.length, results });
}
