...
export async function POST(req: NextRequest) {
  ...
  // on it still being null. If two clicks race, exactly one of them sees
  // a non-zero matched-rows result and proceeds. The other returns 409
  // on the second select-roundtrip.
  ...
  if (!claimed) {
    // Lost the race — another tab already confirmed.
    return NextResponse.json(
      { error: "action already confirmed", ok: false },
      { status: 409 },
    );
