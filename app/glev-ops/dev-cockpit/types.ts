...
// ── Build Execution Plan (Phase 5 + 5.1 hardening) ──────────────────────────

export type Complexity = "low" | "medium" | "high";

/** Core plan returned by Mistral (the deterministic snapshots/metadata are added
 *  by the orchestration, not the model). */
export interface GeneratedBuildPlan {
  scope: string;
  steps: string[];
  affected_areas: string[];
  risks: string[];
...
/**
 * FROZEN build artifact. Created by "Start Build" and stored as JSON in
 * `dev_cockpit_tasks.build_plan` (the latest build, denormalized for the card)
 * AND as a row in `dev_cockpit_builds` (per-build history). PLAN ONLY — no code.
 *
 * The note snapshots are captured at generation time and never re-read from the
 * live queue, so adding/changing queue notes later does NOT mutate an existing
 * build plan.
 *   included_notes_snapshot = queue notes status=applied AND approved_for_current_build
 *   excluded_notes_snapshot = after_build_pending notes (deferred)
 *   (converted_to_task / discarded / separate_task / other tasks are excluded)
 */
export interface BuildExecutionPlan extends GeneratedBuildPlan {
  build_id: string;
  version: number;
  status: TaskStatus;
  included_notes_snapshot: string[];
  excluded_notes_snapshot: string[];
  created_at: string;
  updated_at: string;
}

/** A row in dev_cockpit_builds — one immutable build record (history). */
export interface DevBuild {
  ...
  scope: string | null;
  steps: unknown[];
  included_notes_snapshot: unknown[];
  excluded_notes_snapshot: unknown[];
  affected_areas: unknown[];
  risks: unknown[];
  complexity: string;
  created_at: string;
