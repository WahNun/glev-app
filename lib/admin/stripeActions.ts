 ...
 */
export async function cancelStripeSubAction(formData: FormData): Promise<void> {
  ...
  if (!subId) throw new Error("subscriptionId fehlt");
  if (!["now", "period_end"].includes(mode)) throw new Error("Ungültiger Modus");
  ...
  // Mirror locally so /admin reflects the change immediately even before
  // the customer.subscription.updated webhook arrives.
  try {
 ...
 */
export async function deleteStripeCustomerAction(formData: FormData): Promise<void> {
  const adminToken = await requireAdminToken();
  const customerId = String(formData.get("customerId") ?? "").trim();
  const email = String(formData.get("email") ?? "").trim().toLowerCase() || null;
  if (!customerId) throw new Error("customerId fehlt");
 ...
 * Teilsumme.
 *
 * Beta-Reservations laufen NICHT über dieses Helper — die haben kein
 * Subscription-Objekt. Für Beta-Refunds bitte direkt im Stripe-
 * Dashboard erstatten (oder später einen `refundSessionAction`
 * nachreichen).
 */
export async function refundLatestInvoiceAction(formData: FormData): Promise<void> {
  const adminToken = await requireAdminToken();
  const subId = String(formData.get("subscriptionId") ?? "").trim();
  const email = String(formData.get("email") ?? "").trim().toLowerCase() || null;
  ...
  // Wir wählen explizit die letzte BEZAHLTE Invoice der Subscription
  // mit noch refundierbarem Restbetrag (amount_paid - bisherige
  // Refunds). `subscription.latest_invoice` wäre nicht zuverlässig: bei
  // Renewals zeigt das auf die offene Folge-Rechnung, nicht auf die
  // bereits bezahlte. So refunden wir garantiert echtes Geld und nicht
  // einen offenen Draft.
  ...
  if (candidates.length === 0) {
    throw new Error(
      "Keine bezahlte Invoice für diese Subscription gefunden",
    );
  }
  // Stripe sortiert die Liste bereits desc nach `created`, also ist
  // candidates[0] die zuletzt bezahlte Invoice. Doppelte Refunds /
  // Über-Refunds würden uns Stripe selbst mit einem Fehler ablehnen,
  // daher reicht hier dieser einfache Cap.
  const invoice = candidates[0];
  const amountPaid = invoice.amount_paid ?? 0;

  if (amountCents !== undefined && amountCents > amountPaid) {
    throw new Error(
      `Teilrefund (${(amountCents / 100).toFixed(2)}€) übersteigt bezahlten Betrag (${(
        amountPaid / 100
      ).toFixed(2)}€)`,
    );
  }
  ...
  if (!paymentIntentId) {
    throw new Error("Invoice hat kein Payment Intent (z.B. 0€-Trial)");
  }
...
/**
 * Verlängert die Trial-Phase einer Pro-Subscription um N Tage. Setzt
 * `trial_end` auf max(jetzt, aktuelles trial_end) + N*86400. Danach
 * läuft das Pro-Abo regulär weiter (oder die User:in kündigt selbst).
 */
export async function extendStripeTrialAction(formData: FormData): Promise<void> {
  const adminToken = await requireAdminToken();
  const subId = String(formData.get("subscriptionId") ?? "").trim();
  ...
  if (!Number.isFinite(days) || days < 1 || days > 365) {
    throw new Error("Tage müssen zwischen 1 und 365 liegen");
  }
  ...
  // Mirror trial_ends_at locally so the badge updates without webhook.
  try {
    const sb = getSupabaseAdmin();
    await sb
      .from("pro_subscriptions")
