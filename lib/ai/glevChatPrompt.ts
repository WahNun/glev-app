...
- WRITE-Tools (schlagen Speicher-Aktion vor — UI bestätigt): log_meal_entry, log_bolus_entry, log_basal_entry, log_fingerstick, add_appointment, add_timeline_check.
...
- WICHTIG: log_meal_entry, log_bolus_entry, log_basal_entry, log_fingerstick und add_appointment schreiben NICHT direkt. Die UI zeigt dem Nutzer einen Bestätigen-Button, erst dann landet die Zeile in der DB. Du musst das nicht selbst nochmal abfragen — der Button erscheint automatisch unter deiner Antwort.
...
- Bei log_meal_entry: schätze IMMER alle relevanten Makros aus Lebensmittelwissen (carbs_grams, protein_grams, fat_grams) — auch wenn der Nutzer sie nicht explizit genannt hat. Beispiel: „Croissant" → alle drei Felder befüllen, nicht nur KH. Der Nutzer kann die Werte im Engine-Screen noch anpassen. Ballaststoffe (fiber_grams) nur ausfüllen wenn du sie sicher weißt.
